...
/*****************************************************************************************ADBSCAN********************************************************************/
/**
 * ADBSCAN - Anisotropic Ellipse  Density based clustering
 *
 ...
 * @returns {DBSCAN}
 */
function ADBSCAN(dataset, epsilon, minPts, maxDir, maxEccenChange, distanceFunction) {
  /** @type {Array} */
  this.dataset = [];
  /** @type {number} */
  this.epsilon = 1;
  ...
  this.maxDir = 180;
  /** @type {number} */
  this.maxEccenChange = 0.2;

  // temporary variables used during computation

  /** @type {object} the ellipse of the point, theta in radians*/
  this.ellipse = {"a":this.epsilon, "b":this.epsilon, "theta": 0, "eccentricity": 0};
  /** @type {Array}  all ellipse for all the point*/
  this._ellipseArray = [];
  /** @type {Array} */
  this._visited = [];
  /** @type {Array} */
 ...
 * @returns {DBSCAN}
 */
ADBSCAN.prototype.isInEllipse = function(centerPointId, currentPointId) {
 ...
 * @access protected
 */
ADBSCAN.prototype.ptInEllipse = function(centerPointId) {
  var neighbors = [];
...
/**
 * get the ellipseArray of the every point
 * @param {Array} currentNeighbor, the array contain the id of the points within the Ellipse neighborhood of center point
 * @param {number} centerPointId, 
 * @returns {Array}
 * @access protected
 */
ADBSCAN.prototype.getEllipseArray = function()
{
  for (var currentID = 0; currentID < this._datasetLength; currentID++) {
    this.currentDirectEllipse(currentID);
  }

};
/**
 * get the ellipse of the centriod point
 * @param {Array} currentNeighbor, the array contain the id of the points within the Ellipse neighborhood of center point
 * @param {number} centerPointId, 
 * @returns {Array}
 * @access protected
 */
ADBSCAN.prototype.currentDirectEllipse = function(centerPointId) 
{
  ...
  this.ellipse.eccentricity = 1 - Math.pow(this.ellipse.b, 2)/Math.pow(this.ellipse.a, 2);
  ...
  this._ellipseArray[centerPointId].a = this.ellipse.a;
  this._ellipseArray[centerPointId].b = this.ellipse.b;
  this._ellipseArray[centerPointId].theta = this.ellipse.theta;
  this._ellipseArray[centerPointId].eccentricity = this.ellipse.eccentricity;
...
/**
 * using direction change and eccentricity change to filter the point within the ellipse
 *
 */
ADBSCAN.prototype.directFilterNeighbor = function(Neighbor, centerPointId){
  var centerDir = this._ellipseArray[centerPointId].theta;
  var centerEccen = this._ellipseArray[centerPointId].eccentricity;
  var newNeigbor = [];
  for(var i = 0; i < Neighbor.length; i++)
  {
    var currentDir = this._ellipseArray[Neighbor[i]].theta;
    var currentEccen = this._ellipseArray[Neighbor[i]].eccentricity;
    if((Math.abs(currentDir - centerDir) < this.maxDir || Math.PI - Math.abs(currentDir - centerDir) < this.maxDir) 
      && Math.abs(currentEccen - centerEccen) < this.maxEccenChange)
      newNeigbor.push(Neighbor[i]);
  }
  return newNeigbor;
};

ADBSCAN.prototype._ellipseRegionQuery = function(centerPointId)
{
  var neighbors = this.ptInEllipse(centerPointId);
  var newNeighbors = this.directFilterNeighbor(neighbors, centerPointId);
  return newNeighbors;
}

ADBSCAN.prototype.directionEllipseLoop = function(dataset, epsilon, minPts, distanceFunction) {
  this._init(dataset, epsilon, minPts, distanceFunction);
  for (var id = 0; id < this._datasetLength; id++)
  {
 ...
 * @access protected
 */
ADBSCAN.prototype.localDirectionEllipse = function(centerPointId) {
 ...
 * @access protected
 */
ADBSCAN.prototype._circleRegionQuery = function(pointId) {
  var neighbors = [];
 ...
 * @access protected
 */
ADBSCAN.prototype._euclideanDistance = function(p, q) {
  var sum = 0;
 ...
 * @access public
 */
ADBSCAN.prototype.run = function(dataset, epsilon, minPts, maxDir, maxEccenChange, distanceFunction) {
  this._init(dataset, epsilon, minPts,maxDir, maxEccenChange, distanceFunction);
  this.getEllipseArray();
  // for(var i = 0; i < this._datasetLength; i++)
  // {
  //   console.log(this._ellipseArray[i].theta *180/Math.PI);
  //   console.log(this._ellipseArray[i].eccentricity);
  // }
  ...
  for (var pointId = 0; pointId < this._datasetLength; pointId++) {
    ...
    if (this._visited[pointId] !== 1) {
      ...
      this.ellipse.a = this._ellipseArray[pointId].a;
      this.ellipse.b = this._ellipseArray[pointId].b;
      this.ellipse.theta = this._ellipseArray[pointId].theta;
      this.ellipse.eccentricity = this._ellipseArray[pointId].eccentricity;
 ...
 * @access protected
 */
ADBSCAN.prototype._init = function(dataset, epsilon, minPts, maxDir, maxEccenChange, distance) {
  ...
  if (dataset) {
    ...
    //this._ellipseArray = new Array(this._datasetLength);
    for(var i = 0; i < this._datasetLength; i++)
    {
      this._ellipseArray.push({"a":this.epsilon, "b":this.epsilon, "theta": 0, "eccentricity": 0});
    }
  }
  ...
  if (maxDir) {
    this.maxDir = maxDir * Math.PI/180;
  }

  if (maxEccenChange) {
    this.maxEccenChange = maxEccenChange;
  }
};
 ...
 * @access protected
 */
ADBSCAN.prototype._expandCluster = function(clusterId, neighbors) {
  ...
  for (var i = 0; i < neighbors.length; i++) {
    ...
    if (this._visited[pointId2] !== 1) {
      ...
      this.ellipse.a = this._ellipseArray[pointId2].a;
      this.ellipse.b = this._ellipseArray[pointId2].b;
      this.ellipse.theta = this._ellipseArray[pointId2].theta;
      this.ellipse.eccentricity = this._ellipseArray[pointId2].eccentricity;
      var neighbors2 = this._ellipseRegionQuery(pointId2);

      if (neighbors2.length > this.minPts || neighbors2.length === this.minPts) {
 ...
 * @param {number} clusterId
 */
ADBSCAN.prototype._addToCluster = function(pointId, clusterId) {
 ...
 * @access protected
 */
ADBSCAN.prototype._mergeArrays = function(a, b) {
  var len = b.length;

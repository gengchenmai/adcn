...
var loopAllADParameterSetting = function() {
	for(var eps=1; eps<=100; eps = eps + 5)
	{
...
var loopAllDBParameterSetting = function() {
	...
	for(var eps=1; eps<=100; eps = eps + 1)
	{
		for(var minpts=2; minpts<=20; minpts++)
		{
			...
			ptCluster = dbscan.run(a_crds, eps * GeoScale, minpts, rTree);
...
var loopAllAEParameterSetting = function() {
	...
	for(var eps=1; eps<=100; eps = eps + 1)
	{
		for(var minpts=2; minpts<=20; minpts++)
		{
			...
			ptCluster = aedbscan.run(a_crds, eps * GeoScale, minpts, rTree);
...
var loopAllANParameterSetting = function() {
	...
	for(var eps=1; eps<=100; eps = eps + 1)
	{
		for(var minpts=2; minpts<=20; minpts++)
		{
			...
			ptCluster = andbscan.run(a_crds, eps * GeoScale, minpts, rTree);
...
var loopAllOPTICSParameterSetting = function() {
	...
	for(var eps=1; eps<=100; eps = eps + 1)
	{
		for(var minpts=2; minpts<=20; minpts++)
		{
			...
			ptCluster = optics.run(a_crds, eps * GeoScale, minpts, rTree);
...
var loopDBTime = function(){
	...
	aveTime = aveTime/100;
	var text = $("#PtsNameValue").val();
	var textArray = text.split("-");
	var timeelement = {"Type":textArray[0],"Case":$("#PtsNameValue").val(),"clusterType":"DBSCAN", "Eps": Eps, "MinPts": MinPts, "NumOfPoint":a_crds.length,  "Time":aveTime};
...
var loopAETime = function(){
	...
	var text = $("#PtsNameValue").val();
	var textArray = text.split("-");
	var timeelement = {"Type":textArray[0],"Case":$("#PtsNameValue").val(),"clusterType":"AEDBSCAN", "Eps": Eps, "MinPts": MinPts, "NumOfPoint":a_crds.length, "Time":aveTime};
	ClusterTimeArray.push(timeelement);
...
var loopANTime = function(){
	...
	var text = $("#PtsNameValue").val();
	var textArray = text.split("-");
	var timeelement = {"Type":textArray[0],"Case":$("#PtsNameValue").val(),"clusterType":"ANDBSCAN", "Eps": Eps, "MinPts": MinPts, "NumOfPoint":a_crds.length, "Time":aveTime};
	ClusterTimeArray.push(timeelement);
...
var loopOPTICSTime = function(){
	...
	var text = $("#PtsNameValue").val();
	var textArray = text.split("-");
	var timeelement = {"Type":textArray[0],"Case":$("#PtsNameValue").val(),"clusterType":"OPTICS", "Eps": Eps, "MinPts": MinPts, "NumOfPoint":a_crds.length, "Time":aveTime};
	ClusterTimeArray.push(timeelement);
...
var generateRandomPt = function()
{
	//var numOfPts = Number($("input[name=NumOfPtsValue]").val());
	...
	for(var numOfPts = 500; numOfPts < 10001; numOfPts = numOfPts + 500)
	{
		//var numOfPts = 10000;
		a_crds = new Array(numOfPts);
		for(var j = 0; j < numOfPts; j++)
		{
			a_crds[j] = new Array(2);
			a_crds[j][0] = Math.random() * canvas.width;
			a_crds[j][1] = Math.random() * canvas.height;
		}
		...
		loopANTime();
		loopOPTICSTime();
		console.log(numOfPts + ": Time finished");
...
function openBatchGeoPtFileFn(event){
	...
	for (var i = 0; i < input.files.length; i++) {
	    BatchGeoPtLoopTime(input.files[i]);
	}


	function BatchGeoPtLoopTime(file) {
		//var name = file.name;
...
//open the file selection dislog and read the txt file you select
function openGeoPtFileFn(event){
    ...
    reader.onload = function()
    {
		...
		for (var i = 1; i < array.length; i++) 
		{
			...
			var point = [];
			point.push(coordinateArray[1]);
			point.push(canvas.height - Number(coordinateArray[2]));
			a_crds.push(point);
		};
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		for (var i = 0; i < a_crds.length; i++) 
...
function buildNewRTree(){
	rTree = new RTree();

	// var bounds = {x:0,y:0,w:canvas.width,h:canvas.height};
	// rTree.insert(bounds,-1)
	for(var i=0; i < a_crds.length; i++)
	{
		var bounds = {x:a_crds[i][0],y:a_crds[i][1],w:0,h:0};
		rTree.insert(bounds,i);
	}

	console.log(rTree.get_tree().nodes);
...
$(function() {
	...
	$("#OPTICS").click(function(){
		...
		if($("input[name=EpsValue]").val() !== "" &&$("input[name=MinPtsValue]").val() !== "")
		{
			...
			ptCluster = optics.run(a_crds, Eps, MinPts);
	...
	$("#AEDBSCAN").click(function(){
		...
		if($("input[name=EpsValue]").val() !== "" &&$("input[name=MinPtsValue]").val() !== "")
		{
			...
			for (var i = 0; i < a_crds.length; i++) 
			{
				drawPt(a_crds[i][0], a_crds[i][1], "purple", false, 3);
			};
			//ctx.strokeStyle = "black";
			Eps = Number($("input[name=EpsValue]").val());
			MinPts = Number($("input[name=MinPtsValue]").val());
	...
	$("#ANDBSCAN").click(function(){
		...
		if($("input[name=EpsValue]").val() !== "" &&$("input[name=MinPtsValue]").val() !== "")
		{
			...
			ptCluster = andbscan.run(a_crds, Eps, MinPts);
	...
	$("#LISD").click(function(){
		...
		if($("input[name=EpsValue]").val() !== "" && $("input[name=MinPtsValue]").val() !== "")
		{
			...
			var aedbscan = new AEDBSCAN();
			aedbscan.directionEllipseLoop(a_crds, Eps, MinPts);
			//ptCluster = aedbscan.run(a_crds, Eps, MinPts);
	...
	$("#ADBSCAN").click(function(){
			...
			&& $("input[name=MaxDirChangeValue]").val() !== "" && $("input[name=MaxEccenChangeValue]").val() !== "")
		{
			...
			ptCluster = adbscan.run(a_crds, Eps, MinPts, MaxDirChange, MaxEccenChange);
	...
	$("#RandomGeneratePts").click(function(){
		generateRandomPt();
	});
	...
	$("#run").click(function(){
		...
		if($("input[name=EpsValue]").val() !== "" &&$("input[name=MinPtsValue]").val() !== "")
		{
			...
			console.time("DBSCAN");
			var dbscan = new DBSCAN();
			ptCluster = dbscan.run(a_crds, Eps, MinPts);
	...
	$("#KMEANS").click(function(){
		...
		if($("input[name=KValues]").val() !== "")
		{
			...
			// Eps = Number($("input[name=EpsValue]").val());
			// MinPts = Number($("input[name=MinPtsValue]").val());
			K = Number($("input[name=KValues]").val());
    ...
    $("#saveGeoPtAsFile").click(function(){
		...
		for (var i = 0; i < a_crds.length; i++) 
		{
			var y = canvas.height -a_crds[i][1];
			text += i + "," + a_crds[i][0] + "," + y + "\n";
		};
    ...
    $("#saveLabelGeoPtAsFile").click(function(){
    	var clusterTypeAppend = "";
		if(clusterType === "DBSCAN"){
			clusterTypeAppend = "DB";
		}
		else if(clusterType === "AEDBSCAN"){
			clusterTypeAppend = "AE";
		}
		else if(clusterType === "ANDBSCAN"){
			clusterTypeAppend = "AN";
		}
		else if(clusterType === "OPTICS"){
			clusterTypeAppend = "OP";
		}
		var filename = $("#PtsNameValue").val()+"-"+clusterTypeAppend;
    ...
    $("#saveTimeAsFile").click(function(){
		...
		/*console.log(filename);*/
		var text = "";
		text += "Type" + "," + "Case" + "," + "clusterType" + "," + "Eps" + "," + "MinPts" + ","+ "NumOfPoint"+ ","+ "Time" + "\n";
		for (var i = 0; i < ClusterTimeArray.length; i++) 
		{
			text += ClusterTimeArray[i].Type + "," + ClusterTimeArray[i].Case + "," + ClusterTimeArray[i].clusterType + "," + ClusterTimeArray[i].Eps + "," + ClusterTimeArray[i].MinPts + ","+ ClusterTimeArray[i].NumOfPoint + ","+ ClusterTimeArray[i].Time + "\n";
		};
    ...
    $("#saveIndexAsFile").click(function(){
		...
		for (var i = 0; i < ClusterIndexArray.length; i++) 
		{
			...
			if(ClusterIndexArray[i].Rand > maxRandItem.Rand && ClusterIndexArray[i].NMI !== Infinity)
